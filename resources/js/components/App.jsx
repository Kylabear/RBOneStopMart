...
function App() {
    ...
    return (
        <div className="min-h-screen relative bg-gradient-to-br from-green-50 via-blue-50 to-purple-50">
                    <div className="relative z-10">
                        <Routes>
                {/* Public routes */}
                <Route path="/login" element={!user ? <LoginPage /> : <Navigate to={user.role === 'admin' ? "/admin/dashboard" : "/"} />} />
                <Route path="/register" element={!user ? <RegisterPage /> : <Navigate to={user.role === 'admin' ? "/admin/dashboard" : "/"} />} />
                
                {/* Protected customer routes */}
                <Route path="/" element={user && user.role === 'admin' ? <Navigate to="/admin/dashboard" /> : <Layout><HomePage /></Layout>} />
                <Route path="/products" element={user && user.role === 'admin' ? <Navigate to="/admin/dashboard" /> : <Layout><ProductsPage /></Layout>} />
                <Route path="/products/:id" element={user && user.role === 'admin' ? <Navigate to="/admin/dashboard" /> : <Layout><ProductDetailPage /></Layout>} />
